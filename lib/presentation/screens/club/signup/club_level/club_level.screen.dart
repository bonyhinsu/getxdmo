...
import 'controllers/club_level.controller.dart';

class ClubLevelScreen extends GetView<ClubLevelController>
    with AppBarMixin, AppButtonMixin {
  ClubLevelScreen({Key? key}) : super(key: key);

  final ClubLevelController _controller = Get.find(tag: Routes.CLUB_LEVEL);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: buildAppBar(title: AppString.chooseLevel, centerTitle: true),
      body: SafeArea(
        child: Padding(
  ...
  Widget buildSingleSelectionList() => ListView.separated(
      ...
      itemBuilder: (_, index) {
        final obj = _controller.clubLevel[index];
        obj.itemIndex = index;
      ...
      itemCount: _controller.clubLevel.length);
